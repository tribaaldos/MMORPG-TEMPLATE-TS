...
let wolf = {
  ...
  animation: "Take 001",
  basePosition: [10, 0, 0] as [number, number, number],
  baseQuaternion: [0, 0, 0, 1] as [number, number, number, number], // 👈 rotación inicial
  animSpeed: 1,
};

const players: { [id: string]: { position: [number, number, number] } } = {};

export function registerWolfHandlers(io: Server) {
